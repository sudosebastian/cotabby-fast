...
struct SuggestionRequest: Equatable, Sendable {
    ...
    /// every request is Fresh. Carried on the request so `LlamaGenerationOptions.allowKVReuse`
    /// reflects the Settings toggle without the engine reading UserDefaults.
    ...
    init(
        ...
        self.preferLlamaKVExtend = preferLlamaKVExtend
